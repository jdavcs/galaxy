...
def test_get_user_by_username(session, make_user, make_random_users):
    ...
    user = user_repo.get_user_by_username(session, "a")
...
def test_get_user_by_email(session, make_user, make_random_users):
    ...
    user = user_repo.get_user_by_email(session, "a@foo.bar")
    assert user is my_user
...
def test_get_users_by_ids(session, make_random_users):
    ...
    users2 = user_repo.get_users_by_ids(session, ids)
...
def test_get_users_by_role(session, make_user, make_role, make_user_role_association):
    ...
    role1_users = user_repo.get_users_by_role(session, role1)
    role2_users = user_repo.get_users_by_role(session, role2)
    role3_users = user_repo.get_users_by_role(session, role3)
...
def test_email_exists(session, make_user):
    ...
    assert user_repo.email_exists(session, "a@foo.bar")
    assert not user_repo.email_exists(session, "b@foo.bar")
