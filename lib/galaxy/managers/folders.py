...
class FolderManager:
    ...
    def _get_contained_datasets_statement(
        ...
        if not security.is_admin:  # Non-admin users require ACCESS permission
            ...
            is_public_dataset = is_dataset_public(sa_session, associated_dataset.id, access_action)
...
def is_dataset_public(session, associated_dataset_id, access_action):
    stmt = select(
        not_(
            exists()
            .where(DatasetPermissions.dataset_id == associated_dataset_id)
