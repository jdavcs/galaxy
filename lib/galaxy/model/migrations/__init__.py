...
from sqlalchemy import MetaData, Table, Unicode
...
def run(url, metadata, alembic_dir=None, engine_options=None, app=None, map_install_models=False, auto_migrate=False):
    ...
    dbm = DBManager(url, metadata, ts_metadata, alembic_dir, engine_options) # alembic_dir: ugly!
...
def __backup_run(url, metadata, alembic_dir=None, auto_migrate=False):
...
def __backup_create_galaxy_database(url):
