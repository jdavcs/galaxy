...
class DatasetCollection(Base, Dictifiable, UsesAnnotations, Serializable):
    ...
    def _get_nested_collection_attributes(
        ...
        for entity in return_entities:
            stmt = stmt.add_entity(entity)  # TODO this is bad!
        ...
        # Note: if we return scalars (session.scalars(stmt)), these added columns will NOT be returned.
    ...
    def element_identifiers_extensions_and_paths(self):
        ...
        return [(row[:-2], row.extension, row.Dataset.file_name) for row in nca]
