...
class History(Base, HasTags, Dictifiable, UsesAnnotations, HasName, RepresentById):
    ...
    id = Column(Integer, primary_key=True)
    create_time = Column(DateTime, default=now)
    update_time = Column(DateTime, key='_update_time', index=True, default=now, onupdate=now)
    user_id = Column(Integer, ForeignKey('galaxy_user.id'), index=True)
...
class Dataset(Base, StorableObject, RepresentById, _HasTable):
    ...
    active_history_associations = relationship('HistoryDatasetAssociation',
        primaryjoin=(
            lambda: (Dataset.id == HistoryDatasetAssociation.dataset_id)  # type: ignore
    ...
    active_library_associations = relationship('LibraryDatasetDatasetAssociation',
        primaryjoin=(
            lambda: (Dataset.id == LibraryDatasetDatasetAssociation.dataset_id)  # type: ignore
