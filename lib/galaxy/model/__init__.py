...
class DatasetInstance(UsesCreateAndUpdateTime, _HasTable):
    ...
    def __init__(
        ...
        self.metadata_ = metadat_ or dict()
        self.extended_metadata = extended_metadata
        if (
            dbkey
        ):  # dbkey is stored in metadata_, only set if non-zero, or else we could clobber one supplied by input 'metadata'
    ...
    def get_converted_dataset(self, trans, target_ext, target_context=None, history=None):
        ...
        # See if converted dataset already exists, either in metadata or in conversions.
