...
class GalaxyAppConfiguration(BaseAppConfiguration):
    ...
    def _load_raw_config(self):
        # Load all properties defined in the schema YAML file. TODO process recursively
        # Attributes for nested properties have the form parent-name__child-name
        # (number of levels is arbitrary).
        # raw configuration properties loaded from schema; may be overwritten from passed kwargs
        self._raw_config = {} 
        for key, data in self.schema.properties.items():
    ...
    def _create_attributes_from_raw_config(self):
        """Create an attribute for each property loaded from schema"""
    ...
    def __init__(self, **kwargs):
        ...
        self._update_raw_config_from_kwargs(kwargs)
        self._create_attributes_from_raw_config()
        self._process_config()


        self.database_template = kwargs.get("database_template", 'some default value')

        self.database_template = self._raw_config['database_template']
        ...
        for attr in ATTRIBUTES:
            setattr(self, attr, self._raw_config[name])
