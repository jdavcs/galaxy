...
# config_kwargs are just kwargs!
            if config_kwargs.get(var, None) is not None:
                ...
                for value in values:
                    # resolve to root, and if it exists, that's the path, so IN root.
                    if os.path.exists(os.path.join(self.root, value)):
                        path = value
                        break
                else:
                    # otherwise path is just the value (so NOT in root)
                    path = values[-1]
                setattr(self, var + '_set', False)

            log.debug('to join: root: {}; path: {}'.format(self.root, path))  # doesn't make sense
            # ...unless it's from kwargs.
            
            setattr(self, var, os.path.join(self.root, path))   # and it still ends up in root!!!!!!
...
class GalaxyAppConfiguration(BaseAppConfiguration):
    ...
    def __init__(self, **kwargs):
        ...
        self._mark_config_file_options_if_set(kwargs)  # assign [config-file]_set attributes
    ...
    def _mark_config_file_options_if_set(self, kwargs):
        pass
#        for key in CONFIG_FILE_OPTIONS:
#            attr_name = key + '_set'
#            is_attr_set = getattr(self, option) == self._raw_config[key]
#            setattr(self, attr_name, is_attr_set)
    ...
    def _process_config(self, kwargs):
        ...
        self.oidc_config = self.oidc_config_file
